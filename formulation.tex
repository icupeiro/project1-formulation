...
\begin{table}[h]
	...
	\begin{tabular}{l|c|l|cccc}
		...
		& $T_{z,i}$ & Zone $i$ operative temperatures & - & $\bullet$ & - & - \\
		& $T_{sup, i}$ & Air/water supply temperature of the component $i$ &  - & - & $\bullet$ & - \\
		& $T_{ret, i}$ & Air/water return temperature of the component $i$ &  - & - & $\bullet$ & - \\
		& $T_e$ & Ambient temperature &  - & - & - & $\bullet$ \\
		& $T_b$ & Borefield wall temperature & $\bullet$ & - & - & - \\
		& $T_g$ & Borefield undisturbed ground temperature & $\bullet$ & - & - & - \\
		& $T_{sg, i}$ & Borefield surrounding ground temperature at distance $i$ & $\bullet$ & - & - & - \\
		\midrule
...
\midrule
		...
		$T^{set}_{i}$ & Supply system set-point of the component $i$ & - & - & $\bullet$ & - \\
		& $T^{set}_{z,i}$ & Zone $i$ set-point & - & - & $\bullet$ & - \\
		& $x^{set}_{mov, i}$ & Pump/fan $i$ speed signal & - & - & $\bullet$ & - \\
		& $\Delta p^{set}_i$ & Pump/fan $i$ dif. pressure & - & - & $\bullet$ & - \\
		& $x^{set}_{val, i}$ & Valve $i$ position & - & - & $\bullet$ & - \\
		& $x^{set}_{dam, i}$ & Damper $i$ position & - & - & $\bullet$ & - \\
		\midrule
		\multirow{4}{*}{\textbf{\shortstack[l]{Indoor Environmental \\ Quality (IEQ)}}} &
		$PMV_i$ & Predicted mean vote in zone $i$ & - & $\bullet$ & - & - \\
		& $CO_{2,i}$ & $CO_2$ concentration in air in zone $i$ & - & $\bullet$ & - & $\bullet$ \\
		& $\phi_i$ & Humidity in zone $i$& - & $\bullet$ & - & $\bullet$ \\
		& $E_{v,i}$ & Illuminance in zone $i$ & - & $\bullet$ & - & - \\
		\bottomrule 
	\end{tabular}
\end{table} 
...
\begin{table}[h]
	...
	\begin{tabular}{l|c|l|c}
		\toprule
		\textbf{Efficiencies}  & \textbf{Symbol} &  \textbf{Description} & \textbf{Associated variables} \\
		\midrule
		\makecell[l]{Coefficient of \\ performance} & $COP_i$ &  \makecell[l]{Heating efficiency of heat pump \\ or air conditioning system $i$} & $T_{sup}, Q_{i}, P_{i}$ \\
		\makecell[l]{Energy efficiency \\ ratio} & $EER_i$ & \makecell[l]{Cooling efficiency of heat pump \\ or air conditioning system $i$} & $T_{sup,i}, Q_{i}, P_{i}$ \\
		Efficiency & $\eta$ & \makecell[l]{Efficiency of all other systems} & $T_{sup,i}, Q_{i}, P_{i}, ...$ \\
		\midrule
		\textbf{Pricing factor}  & \textbf{Symbol} &  \textbf{Description} \\
		\midrule
		Price factor & $p_i$ &  \makecell[l]{Conversion factor from energy \\ to monetary cost of component $i$} & $Q_i$ \\
		Emission factor & $e_i$ & \makecell[l]{Conversion factor from energy \\ to $CO_2$ emissions of component $i$} & $Q_i$ \\
		RES share factor & $r_i$ & \makecell[l]{Ratio of renewable energy of \\ the considered component $i$} & $Q_i$ \\
		\midrule
		\textbf{Comfort bounds}  & \textbf{Symbol} &  \textbf{Description} \\
		\midrule
		Temperature & $\underline{T},\overline{T}$ & \makecell[l]{Operative temperature comfort \\ bounds of zone $i$} & $T_{z,i}$ \\
		$CO_2$ concentration & $\underline{CO_2},\overline{CO_2}$ & \makecell[l]{$CO_2$ concentration comfort \\ bounds of zone $i$} & $CO_{2,i}$ \\
		Predicted mean vote & $\underline{PMV},\overline{PMV}$ &  \makecell[l]{Predicted mean vote comfort \\ bounds of zone $i$} & $PMV_i$ \\
		Humidity & $\underline{\phi},\overline{\phi}$ &  Humidity comfort bounds & $\phi_i$  \\
		Illuminance & $\underline{E_v},\overline{E_v}$ & Illuminance comfort bounds & $E_{v,i}$  \\
		\bottomrule 
		\textbf{Component limitations}  & \textbf{Symbol} &  \textbf{Description} \\
		\midrule
		Power limit & $\underline{Q_i},\overline{Q_i}$ & \makecell[l]{Minimum/maximum power of \\ the HVAC component $i$} & $Q_i$ \\
		Power rate of change & $\underline{\Delta Q_i},\overline{\Delta Q_i}$ & \makecell[l]{Minimum/maximum power rate of \\ change of the HVAC component $i$} & $Q_i$ \\
		\midrule
		\textbf{Auxiliary parameters}  & \textbf{Symbol} &  \textbf{Description} \\
		\midrule
		\makecell[l]{Arbitrary weighting \\ term} & $Q_{\ell}$ &  \makecell[l]{Weighting term for the $\ell$ term in \\ the objective function} \\
		\makecell[l]{Slack variable} & $s$ &  \makecell[l]{Used to soften the constraints, usually \\ for the given comfort requirement} \\
		\makecell[l]{Time-step} & $t_s$ &  \makecell[l]{Time-step used in the \\ optimization problem} \\
		\bottomrule 
	\end{tabular}
\end{table}
