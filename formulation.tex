...
The bjective function is given by~\eqref{eq:mpc_general_formal:cost} where   $\ell_N(x_N)$  represents the terminal penalty,
...
\begin{table}[h]
	...
	\begin{tabular}{l|c}
		...
		\midrule
		Minimise energy use &   $  \min\sum_{i=1}^{n_u} \dfrac{\dot{Q}^{i}_{\text{HVAC}}}{\eta^i}$ \\
		Minimise cost & $  \min\sum_{i=1}^{n_u} p^i \dfrac{\dot{Q}^{i}_{\text{HVAC}}}{\eta^i}$  \\
		Minimise $CO_2$ emissions & $ \min\sum_{i=1}^{n_u}  e^i \dfrac{\dot{Q}^{i}_{\text{HVAC}}}{\eta^i}$  \\
		Maximise RES &  $ \min\sum_{i=1}^{n_u} (1-r^i) \dfrac{\dot{Q}^{i}_{\text{HVAC}}}{\eta_i}$  \\
		Flexibility & TBD  \\
		Minimize discomfort &  $ \min\sum_{i=1}^{n_y} ||s^i||^2_2$ \\
		\bottomrule 
	\end{tabular}
\end{table}
